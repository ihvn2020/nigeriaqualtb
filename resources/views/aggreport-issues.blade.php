...
@section('content')
    ...
    <h3 class="page-title" style="clear: both; position: relative">Indicator Issues for: <small
            style="color: green">{{ $aggreportissues->first()->aggreport->title }} </small></h3>
    <div class="row">

        <div class="panel">

            <div class="panel-heading">
                <b>Reporting State/Facility:</b>
                {{ $aggreportissues->first()->aggreport->faciliti->state . ' / ' . $aggreportissues->first()->aggreport->faciliti->facility_name }}
            </div>
                    ...
                    <tbody>
                        @foreach ($aggreportissues as $agri)
                            <tr>
                                ...
                                <td>
                                    <ul>
                                        @foreach ($agri->qiactivities as $qia)
                                            <li>{{ $qia->activities }} | <i><small
                                                        style="color:green">{{ $qia->enteredby->name }}-
                                                        {{ $qia->dated }}</small></i></li>
                                        @endforeach
                                    </ul>
                                </td>
                                        ...
                                        <form action="{{ route('saveqicomment') }}" method="POST">
                                            ...
                                            <div class="row">
                                                <div class="form-group col-md-10">
                                                    <input type="text" name="comments" placeholder="enter comment & save"
                                                        class="form-control">
                                                </div>
                                ...
                                <th><a href="#" id="iid{{ $agri->id }}" class="btn btn-primary btn-sm newqi"
                                        data-toggle="modal" data-target="#newqi" data-issue_id="{{ $agri->id }}">Add
                                        QI</a></th>
                ...
                <div class="modal-body">
                    ...
                    <form method="POST" action="{{ route('saveqi') }}">
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="activities" class="form-control" id="activities" rows="5" maxlength="250"
                                placeholder="Upto 250 characters"></textarea>
                        </div>
